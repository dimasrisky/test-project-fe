...
export default function Profile() {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_PREFIX_BACKEND_URL}/auth/me`, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('accessToken')
      const response = await fetch(`${import.meta.env.VITE_PREFIX_BACKEND_URL}/user/profile`, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        },
  ...
  const handleToggle2FA = async () => {
    if(!twoFAEnabled){
      ...
      try {
        const token = localStorage.getItem('accessToken')
        const response = await fetch(`${import.meta.env.VITE_PREFIX_BACKEND_URL}/user/enable-tfa`, {
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
          },
      ...
      try {
        const token = localStorage.getItem('accessToken')
        const response = await fetch(`${import.meta.env.VITE_PREFIX_BACKEND_URL}/user/disable-tfa`, {
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
          },
  ...
  const handleLogout = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_PREFIX_BACKEND_URL}/auth/logout`, {
        headers: { 
          'Content-Type': 'application/json',
